...
func TestFanUseCase_CreateFan(t *testing.T) {
	...
	uc := NewFanUseCase(repo, &noopAuditLogger{})
	resp, err := uc.CreateFan(ctx, dto.CreateFanRequest{Name: "FAN A", Level: "junior"})
...
func TestFanUseCase_GetFan(t *testing.T) {
	...
	ucFail := NewFanUseCase(repoFail, &noopAuditLogger{})
